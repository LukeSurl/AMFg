...
!------------------------------
   ...
   subroutine input_newGOME(satellite_file,TG_TYPE,ULN,LAT_centers,LON_centers,SZA,SVA,AZM,PCLD,FCLD,TCOT,SC,MAXLINE,outPrefix,outSuffix,flag)
      ...
      !INTEGER, intent(out) :: nullread(17)                  ! A scratch variable to be written to then ignored. 
      INTEGER, intent(out) :: MAXLINE                       ! The number of observations read in
      CHARACTER(LEN=200),intent(out) :: outSuffix(NVALMAX)  ! A string to pass on data from satellite file to AMF output
      CHARACTER(LEN=100),intent(out) :: outPrefix(NVALMAX)  ! A string to pass on data from satellite file to AMF output
      LOGICAL, intent(out) :: flag(NVALMAX)       !Satellite flag, do not use measurement if true
      ...
      !local variables
      REAL  holder(31) ! a variable to hold the data before assigning it to output
      INTEGER xx
      ...
      DO LINE = 1, NVALMAX
      !   READ(LUN, '(i9,i5,i3,14f8.3,2f9.3,3e11.3,a11)', END=901) &
      !     ULN(LINE),PIXEL(LINE), SCAN(LINE),             &
      !     SZA(LINE), SVA(LINE),                          &
      !     AZM(LINE),				          &
      !     I1(LINE), J1(LINE),				  &
      !     I2(LINE), J2(LINE),                            &
      ...
      !     SC(LINE), DSC(LINE), FITRMS(LINE),             &
      !     TIME(LINE)                                     
          READ(LUN,*,END=901) (holder(xx),xx=1,31)
          ...
          SVA(LINE)=holder(6)
          AZM(LINE)=holder(7)
          LAT_centers(LINE)=holder(8)
          LON_centers(LINE)=holder(9)
          FCLD(LINE)=holder(10)
          PCLD(LINE)=holder(11)
          SC(LINE)=holder(12)
          DSC(LINE)=holder(13)
          !nullread(0),&!average column uncertainty
          !nullread(1),&!average fitting RMS
          !nullread(2),&!Pre-destripting column amount
          !nullread(3),&!Column uncertainty
          !nullread(4),&!Fit convergence flag
          FITRMS(LINE)=holder(19) !Fitting RMS
          !nullread(5),&!Main data quality flag
          !nullread(6),&!Maximum column amount
          !nullread(7),&!Radiance reference column amount
          !nullread(8),&!Radiance reference column uncertainty
          !nullread(9),&!Radiance reference column XTR fit
          !nullread(10),&!Radiance reference fitting RMS
          !nullread(11),&!Radiance reference latitude range SOUTH
          !nullread(12),&!Radiance reference longitude range NORTH
          !nullread(13),&!Radiance WavCal convergence flag
          !nullread(14),&!Radiance WavCal latitude range SOUTH
          !nullread(15),&!Solar WavCol convergence flag
          !nullread(16)  !XtrackQualityFlag
             
          TCOT(LINE) = 0.0 !not used, placeholder value.
      ENDDO
